...
10. **摘要与表格数字必须一致**：凡 ai_summary 中出现的营收、净利润、同比增速等关键数字，必须同步写入 kpis 或 income_statement 对应 period（不得只写在摘要里）；金额一律用 millions（百万美元），中文「亿美元」= 数值×100 millions（例：47亿美元 → revenue.value 为 4700）
11. 若原文为财年/全年（FY）数据而非单季，仍用本报告财季「{fiscal_period}」作为键存放该期可用数据（可视为 FY 期末或最近一期），禁止因不确定单季而留空整个 kpis/income_statement
12. red_flags：文中所有重要风险（云依赖、诉讼、可转债、持续亏损等）须逐条写入本数组 {{code, message}}，**不要**只在 ai_summary 里概括；无则 []
13. material_events：文中较大的盈利或亏损/费用冲击事项（一次性税项收益、减值、重组、大额投资收益/公允价值变动等），数组元素含 type（profit 或 loss）、title、amount_millions（百万美元，文中无则 null）、period（YYYY-Qn，无则 null）、description；仅收录文中明确或可合理推断的重大事项，最多 8 条，无则 []
14. ai_summary：3-5 句白话「公司体检报告」（可与表格重复叙述，但表格仍须有对应数字）
15. 仅返回 JSON，不要 markdown 代码块
