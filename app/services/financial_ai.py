...
11. material_events：文中较大的盈利或亏损/费用冲击事项（一次性税项收益、减值、重组、大额投资收益/公允价值变动等），数组元素含 type（profit 或 loss）、title、amount_millions（百万美元，文中无则 null）、period（YYYY-Qn，无则 null）、description；仅收录文中明确或可合理推断的重大事项，最多 8 条，无则 []
12. ai_summary：3-5 句白话「公司体检报告」
13. 仅返回 JSON，不要 markdown 代码块
